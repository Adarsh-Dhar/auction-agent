...
export function AuctionDetailView({ auctionId = "AUC-1048" }: Props) {
  ...
  const formatEndsAt = (endsAt: string | null, deadline?: number) => {
    // Use bidding round deadline if available, otherwise fall back to endsAt
    const endTime = deadline || (endsAt ? Date.parse(endsAt) : null)
    if (!endTime) return "No end date"
    if (Number.isNaN(endTime)) return "Invalid date"
    const now = Date.now()
...
function AuctionCard({ auction }: { auction: Auction }) { return <a className="market-card" href={`/auctions/${auction.id}`}><div className="auction-art"><span>AUCTION / {auction.id}</span><span>↗</span></div><div className="card-top"><span className={badgeClass(auction.status)}>{auction.status}</span><span>{auction.bidders} bidders</span></div><h3>{auction.title}</h3><div className="card-metrics"><span>Top bid <b>{auction.topBid}</b></span><span>Floor <b>{auction.floor}</b></span></div><div className="card-footer"><span>{auction.channels.join(" + ")}</span><span>Ends {auction.endsAt ? new Date(auction.endsAt).toLocaleDateString() : "No end date"}</span></div></a> }
function NewAuctionWizard({ onClose, onCreated }: { onClose: () => void; onCreated: (auction: Auction) => void }) {
  const [step, setStep] = useState(1)
  const [form, setForm] = useState({ title: "", floor: "", terms: "", autoExtend: true, requiresApproval: true, minIncrement: "", hasEndTime: true, endsAt: "" })
  const [created, setCreated] = useState<Auction | null>(null)
  const [copied, setCopied] = useState(false)
  const update = (key: string, value: string | boolean) => setForm((current) => ({ ...current, [key]: value }))
  
  // Calculate default min increment as 1% of floor
  useEffect(() => {
    if (form.floor) {
      const floorValue = parseFloat(form.floor.replace(/[^0-9.]/g, ""))
      if (!isNaN(floorValue) && floorValue > 0) {
        const calculated = (floorValue * 0.01).toFixed(2)
        setForm(prev => ({ ...prev, minIncrement: `$${calculated}` }))
      }
    }
  }, [form.floor])
  
  const create = async () => {
    const endsAt = form.hasEndTime ? (form.endsAt || new Date(Date.now() + 7 * 86400000).toISOString()) : null
    const response = await fetch("/api/auctions", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...form, floor: `$${form.floor || "0"}`, status: "draft", endsAt, minIncrement: form.minIncrement || "$1", channels: ["Web chat"] }) })
  ...
  return <div className="modal-backdrop"><div className="wizard modal-card"><div className="modal-heading"><div><span className="eyebrow">New auction · step {Math.min(step, 4)} of 4</span><h2>{step === 1 ? "Describe the lot" : step === 2 ? "Set guardrails" : step === 3 ? "Preview the live phrasing" : "Share the room code"}</h2></div><button className="icon-button" onClick={step === 4 ? finish : onClose}>×</button></div>
    ...
    {step === 2 && <div className="form-stack"><label>Minimum bid increment<input value={form.minIncrement} onChange={(e) => update("minIncrement", e.target.value)} placeholder="Auto-calculated as 1% of floor" /></label><Toggle label="Set a closing time" value={form.hasEndTime} onChange={(value) => update("hasEndTime", value)} />{form.hasEndTime && <label>End time<input type="datetime-local" value={form.endsAt} onChange={(e) => update("endsAt", e.target.value)} /></label>}<Toggle label="Auto-extend closing window" value={form.autoExtend} onChange={(value) => update("autoExtend", value)} /><Toggle label="Require operator approval for exceptions" value={form.requiresApproval} onChange={(value) => update("requiresApproval", value)} /><div className="policy-note">Bids below the floor will be rejected. New bidder requests are classified before acceptance.</div></div>}
    {step === 3 && <div className="preview-card"><span className="live-indicator"><i /> Live phrasing preview</span><p>“{form.title || "Your auction lot"} is open. Bids start at {form.floor ? `$${form.floor}` : "$0"} and must increase by at least {form.minIncrement || "$1"} each time (1% of floor). {form.hasEndTime ? (form.endsAt ? `The auction closes on ${new Date(form.endsAt).toLocaleDateString()}.` : "The auction closes at a scheduled time.") : "There is no end time — you'll close it manually."} {form.autoExtend ? "The closing window extends when a bid arrives near close." : ""}”</p><small>{form.requiresApproval ? "Operator approval is enabled for unusual requests." : "The agent can handle standard requests automatically."}</small></div>}
